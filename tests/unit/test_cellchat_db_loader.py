...
# ── skip guard ─────────────────────────────────────────────────────────────────
# The CSV files are generated by scripts/export_cellchat_db.R.  Skip all
# CSV-dependent tests gracefully if the files are not yet present.

def _data_dir_exists(species: str) -> bool:
    try:
        pkg = f"py_cellchat.database.data.{species}"
        ref = resources.files(pkg).joinpath("interaction.csv")
        return ref.is_file()
...
def test_load_cellchat_db_invalid_species_raises():
