...
def _make_minimal_db() -> CellChatDB:
    return CellChatDB(interaction_input=pd.DataFrame({
        "ligand":     ["L1"],
        "receptor":   ["R1"],
...
def test_subset_db_annotation_default(human_db):
    ...
    annotations = set(result.interaction_input["annotation"])
    ...
    assert len(result.interaction_input) > 0
...
def test_subset_db_non_protein_true(human_db):
    ...
    assert len(result_with_non_protein.interaction_input) >= len(result_default.interaction_input)
...
def test_subset_db_custom_key(human_db):
    ...
    assert set(result.interaction_input["pathway_name"]) == {"CXCL"}
    assert len(result.interaction_input) > 0
...
def test_subset_db_multi_key(human_db):
    ...
    assert set(result.interaction_input["annotation"]) == {"Secreted Signaling"}
    assert set(result.interaction_input["pathway_name"]) == {"CXCL"}
...
def test_subset_db_original_unchanged(human_db):
    original_len = len(human_db.interaction_input)
    subset_db(human_db)
    assert len(human_db.interaction_input) == original_len
...
def test_subset_db_invalid_key_raises():
    db = _make_minimal_db()
    with pytest.raises(ValueError, match="not found in interaction_input"):
...
def test_subset_db_non_protein_in_search_enables_flag(human_db):
    ...
    if "Non-protein Signaling" in set(human_db.interaction_input["annotation"]):
